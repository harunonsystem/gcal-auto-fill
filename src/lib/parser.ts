...
const MEETING_URL_REGEX = /https:\/\/(?:[a-zA-Z0-9-]+\.)?(?:zoom\.us|webex\.com|teams\.microsoft\.com|meet\.google\.com|slack\.com\/huddle)\/[^\s<>]+/gi;
...
export function parseDateTime(text: string): ParsedDateTime | null {
  ...
  const end = best.end?.date() || new Date(start.getTime() + DEFAULT_DURATION_MS);
